...
class Install extends Command
{
     ...
     * @var string
     */
    protected $signature = 'xylophone:install
     ...
     * @var string
     */
    protected $description = 'Install Xylophone requirements on dev, publish files and create uploads directory.';
    ...
    {
        /*
        * "ask" comes by default, when no option is provided, like: "xylophone:install"
        ...
        */
        ...
        $this->info(' Xylophone installation started. Please wait...');
        $this->progressBar->advance();

        $this->line(' Publishing configs, langs, views, js and css files');
        $this->executeProcess('php artisan vendor:publish --provider="Xylophone\CRUD\XylophoneServiceProvider" --tag="minimum"');
        ...
        $this->executeProcess('php artisan xylophone:publish-user-model');

        $this->line(" Creating App\Http\Middleware\CheckIfAdmin.php");
        $this->executeProcess('php artisan xylophone:publish-middleware');
        ...
        if ($install_elfinder) {
            ...
            $this->executeProcess('php artisan vendor:publish --provider="Xylophone\CRUD\XylophoneServiceProvider" --tag="elfinder"');

            $this->line(' Adding sidebar menu item for File Manager');
            switch (DIRECTORY_SEPARATOR) {
                case '/': // unix
                    $this->executeProcess('php artisan xylophone:add-sidebar-content "<li class="nav-item"><a class="nav-link" href=\"{{ xylophone_url(\'elfinder\') }}\"><i class=\"nav-icon fa fa-files-o\"></i> <span>{{ trans(\'xylophone::crud.file_manager\') }}</span></a></li>"');
                    break;
                case '\\': // windows
                    $this->executeProcess('php artisan xylophone:add-sidebar-content "<li class="nav-item"><a class="nav-link" href=""{{ xylophone_url(\'elfinder\') }}""><i class=""nav-icon fa fa-files-o""></i> <span>{{ trans(\'xylophone::crud.file_manager\') }}</span></a></li>"');
                    break;
        ...
        $this->info(' Xylophone installation finished.');
    }
