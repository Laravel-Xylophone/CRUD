...
trait SaveActions
{
    ...
    {
        return config('xylophone.crud.operations.'.$this->getCurrentOperation().'.defaultSaveAction', 'save_and_back');
    }
    ...
    public function setSaveAction($forceSaveAction = null)
    {
        ...
        $showBubble = $this->getOperationSetting('showSaveActionChange') ?? config('xylophone.crud.operations.'.$this->getCurrentOperation().'.showSaveActionChange') ?? true;

        if ($showBubble &&
            session($this->getCurrentOperation().'.saveAction', 'save_and_back') !== $saveAction) {
            \Alert::info(trans('xylophone::crud.save_action_changed_notification'))->flash();
        }
    ...
    private function getSaveActionButtonName($actionValue = 'save_and_back')
    {
        switch ($actionValue) {
            case 'save_and_edit':
                return trans('xylophone::crud.save_action_save_and_edit');
                break;
            case 'save_and_new':
                return trans('xylophone::crud.save_action_save_and_new');
                break;
            case 'save_and_back':
            default:
                return trans('xylophone::crud.save_action_save_and_back');
                break;
        }
    }
}
