...
@foreach($revisions as $revisionDate => $dateRevisions)
      <h5 class="text-primary">
        {{ Carbon\Carbon::parse($revisionDate)->isoFormat(config('xylophone.base.default_date_format')) }}
      </h5>
    ...
    <div class="card">
      ...
      @if($history->key == 'created_at' && !$history->old_value)
        <div class="card-header">
          ...
          {{ $history->userResponsible()?$history->userResponsible()->name:trans('xylophone::crud.guest_user') }} {{ trans('xylophone::crud.created_this') }} {{ $crud->entity_name }}
        </div>
      @else
        <div class="card-header">
          <strong class="time"><i class="fa fa-clock-o"></i> {{ date('h:ia', strtotime($history->created_at)) }}</strong> -
          {{ $history->userResponsible()?$history->userResponsible()->name:trans('xylophone::crud.guest_user') }} {{ trans('xylophone::crud.changed_the') }} {{ $history->fieldName() }}
          <div class="card-header-actions">
            <form class="card-header-action" method="post" action="{{ url(\Request::url().'/'.$history->id.'/restore') }}">
              ...
              <button type="submit" class="btn btn-outline-danger btn-sm restore-btn" data-entry-id="{{ $entry->id }}" data-revision-id="{{ $history->id }}" onclick="onRestoreClick(event)">
                <i class="fa fa-undo"></i> {{ trans('xylophone::crud.undo') }}</button>
              </form>
          </div>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-md-6">{{ mb_ucfirst(trans('xylophone::crud.from')) }}:</div>
            <div class="col-md-6">{{ mb_ucfirst(trans('xylophone::crud.to')) }}:</div>
          </div>
          <div class="row">
    ...
    function onRestoreClick(e) {
      ...
      $.ajax('{{ \Request::url().'/' }}' +  revisionId + '/restore', {
        ...
        success: function(revisionTimeline) {
          ...
          new Noty({
              type: "success",
              text: "{{ trans('xylophone::crud.revision_restored') }}"
          }).show();
        }
      });
  }
