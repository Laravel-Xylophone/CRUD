<nav class="navbar navbar-expand-lg navbar-filters mb-0 pb-0 pt-0">
      ...
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#bp-filters-navbar" aria-controls="bp-filters-navbar" aria-expanded="false" aria-label="{{ trans('xylophone::crud.toggle_filters') }}">
        <span class="fa fa-filter"></span> {{ trans('xylophone::crud.filters') }}
      </button>
        ...
        <ul class="nav navbar-nav">
          <!-- THE ACTUAL FILTERS -->
    			@foreach ($crud->filters() as $filter)
    				@include($filter->view)
    			@endforeach
          <li class="nav-item"><a href="#" id="remove_filters_button" class="nav-link {{ count(Request::input()) != 0 ? '' : 'invisible' }}"><i class="fa fa-eraser"></i> {{ trans('xylophone::crud.remove_filters') }}</a></li>
        </ul>
      </div><!-- /.navbar-collapse -->
  </nav>
